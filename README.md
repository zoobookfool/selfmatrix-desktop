...
- **shell 側の実装 (`src/`)**:
  ...
  - `minisign-nsis-updater.cjs` — stock `NsisUpdater` を拡張し、installerと同じreleaseから `<installer>.minisig` を取得して埋め込み公開鍵で必ず検証する。pending cacheは毎回クリアし、以前の未検証installerが再利用される経路も閉じる。
  - `window-bounds.cjs` — モニター着脱やDPI変更後の通話窓を現行work areaへ復元する純関数。通常起動はsingle-instance lockを取得し、2個目の起動は既存main windowを前面化して終了する。
...
CI/releaseが採用するCinny/Element Callは [`product-lock.json`](product-lock.json) の完全なcommit SHAで固定する。ローカルの兄弟checkoutとlockの整合は `node scripts/release-inputs.cjs --verify-siblings` で確認できる。
...
```powershell
npm test              # 通常のsmoke/probe一式 + updater + single-instance + window bounds
npm run smoke
npm run memory
npm run cinny-shell-smoke
npm run tray-probe    # トレイ常駐 (M2)。OS の実クリックは自動化できないため、close-to-tray/
                       # メニュー/クリックハンドラをプログラム的に検証する専用モード
npm run update-download-probe          # 実NsisUpdaterで署名正常/欠落/改ざんを検証
npm run probe:single-instance          # 同じuserDataで2プロセス起動し、2個目の副作用が無いことを検証
npm run probe:window-bounds            # モニター着脱/DPI/巨大・微小boundsの復元
npm run package:probe:update-download  # unpacked製品を生成し、製品実行ファイルからupdaterを検証
...
npm run e2e:callflow   # alice/bob 2 ユーザー通話 + 画面共有 + 窓移動再接続 (10 往復) + 実 ⧉ ボタン
                        # popout/popin + 別窓 close=メイン復帰 + call-control 7 語彙 + 通話跨ぎ回帰
```
...
**SelfMatrix M3 step 5 (M3 の受け入れ条件そのもの、design/m3-window-ux.md §4)**: 通話中の窓出し入れが 10 往復で切断ゼロであること (`passConditions.windowMoveReparenting`、`REPARENT_ROUND_TRIPS`) に加え、以下 2 点を実測する。

- `passConditions.realPopoutPopinClick` — cinny 自身の実 ⧉ ボタン (`call_popout`/`call_popin`) を Playwright で実クリックし、production 配線 (`CallControls.tsx` の onClick → `useNativeCallPopoutToggle()` → `NativeCallEmbed.popout()`/`popin()` → transport → IPC → `main.cjs` の `detachCallView()`/`attachCallView()`) をエンドツーエンドで検証する。`windowMoveReparenting`/`footerVisibilityToggle` は `__selfmatrixE2E.detachCallView()`/`attachCallView()` を直接呼ぶため cinny 側の TS 層を経由しない — この節がその穴を塞ぐ。
- `passConditions.closeWindowMainRevert` — 別窓をユーザーが実際に閉じたとき (`__selfmatrixE2E.closeCallWindow()` = `state.callWindow.close()`、`win.destroy()` ではない実 X ボタンと同じ経路) の「メインへの自動復帰・無再接続 (RTCPeerConnection id 集合不変・connected 維持)・通話継続」を実測する。最重要判定は `callViewState !== "none"` (= closeCallView()/hangup が誤発火して通話が終わっていないことの確認、dispose 誤発火検知)。bob 側は無影響であることも確認する。
...
`npm test` / `npm run e2e:*` / 各種 probe は `evidence/*.json` (+ スクリーンショット) にローカルで結果を書き出すが、このリポジトリでは `.gitignore` によりコミット対象外にしている。push/PRでは `.github/workflows/ci.yml` が固定済み3リポジトリを組み合わせて同じ製品ゲートを再実行する。
